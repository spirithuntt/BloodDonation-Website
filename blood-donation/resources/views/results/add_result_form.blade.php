         ...
         <div class="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <form method="POST" action="{{route('results.store')}}" class="max-w-3xl mx-auto" data-parsley-validate>
                        @csrf
                            ...
                            @foreach ($tests as $test)
                                <div class="bg-white shadow-md rounded-md p-6">
                                    ...
                                    @if ($test->result_type == 'number')
                                        <input type="text" name="result_number[{{$test->id}}]" id="test_{{$test->id}}" class="mt-1 block w-full rounded-md bg-gray-100 border-transparent focus:border-gray-500 focus:bg-white focus:ring-0" required data-parsley-required data-parsley-type="number" >
                                    @else
                                        <select name="passed[{{$test->id}}]" id="test_{{$test->id}}" class="mt-1 block w-full rounded-md bg-gray-100 border-transparent focus:border-gray-500 focus:bg-white focus:ring-0" required data-parsley-required>
